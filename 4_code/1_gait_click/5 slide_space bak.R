...
for(analysis in c('left','right')){
        ...
        p <- ggplot(dat_intac, aes(x=x,y=y,group=no)) +
                ...
                #geom_line() +
                scale_linetype_manual(values=c(2,2,1,1)) +  # (ci1, ci2, edge, slide)
                scale_size_manual(values=c(0.2,0.2,0.2,0.8)) +
                scale_colour_manual(values=c('blue','green','orange','pink','red','black')) + #(sp1,sp2,sp3,sp4,sp5,edge)
                xlim(20,350) +
                #ylim(300,100) +
                ylim(350,20) +
                ggtitle(paste0('Slide space: ', analysis))
        
        png(paste0("4_results/interaction_feature/space/","slide_space_",analysis,".png"),width = 2000,height = 1800,res = 300)
        print(p)
        dev.off()
