tense_data_more2 <- function(tenseDATA){
        dat <- tenseDATA
        disX = 45
        disY = 26
        pointx <- c(45,90,135,180,225,270,315)
        pointx <- c(pointx, pointx+disX/2)
        pointy <- c(25.5,51.5,77.5,103.5,129.5,155.5,181.5,207.5,233.5,259.5,285.5,311.5)
        pointy <- c(pointy, pointy+disY/2)
        ...
        for(i in 1:nrow(dataF)){
                ...
                if(length(indexCOM) > 0){
                        ...
                        indexOLD = 0
                        indexL = which((dat$Lon == xDOWN) & (dat$Lat == yM)) # find left point
                        indexR = which((dat$Lon == xUP) & (dat$Lat == yM)) # find right point
                        indexU = which((dat$Lon == xM) & (dat$Lat == yUP)) # find up point
                        indexD = which((dat$Lon == xM) & (dat$Lat == yDOWN)) # find down point
