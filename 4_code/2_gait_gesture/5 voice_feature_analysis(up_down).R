...
feature_voice = read.csv('4_results/gait_gesture/feature_3voice.csv', header=FALSE)
names(feature_voice) = c('iperson', 'isp', 'all_time', 'all_time_key', 'rise_time', 'fall_time', 'angX_changed', 'angY_changed', 'angZ_changed', 'accxyzUP', 'accxyzDOWN', 'length_new')
...
sp = boxplot(dat$all_time_key)$out # all time
idxOUT_all_time_key = vector()
for (k in 1:length(sp)){
        idxOUT_all_time_key = c(idxOUT_all_time_key, which(dat$all_time_key==sp[k]))
}

sp = sort(boxplot(dat$rise_time)$out) # rise time
idxOUT_rise_time = vector()
for (k in (length(sp)-5):length(sp)){
...
for (k in (length(sp)-4):length(sp)){
        idxOUT_fall_time = c(idxOUT_fall_time, which(dat$fall_time==sp[k]))
}

idxOUT_len1 = which((dat$length_new == 0))
idxOUT_len2 = which((dat$length_new > 1))
idxOUT_len = c(idxOUT_len1,idxOUT_len2)
...
for(i in c('time','acc')){
        ...
        png(paste0("4_results/gait_gesture/Fig_AOV_3voice/", 'z_Phase_Arm_', i,".png"), width = 1200, height = 1000, res = 300)
        print(p)
        dev.off()
        ...
        p <- ggplot(resultsSE, aes(x=vectorX, y=vectorY, group=1)) +
                ...
                scale_x_discrete(name='Phase of Arm',
                                 ...
                                 labels = c("rise" = "Rise","fall" = "Fall")) +
                scale_y_continuous(name="Value")
        png(paste0("4_results/gait_gesture/Fig_AOV_3voice/0paper_", 'z_Phase_Arm_', i,".png"), width = 1000, height = 800, res = 300)
        print(p)
        dev.off()
}
###########################################