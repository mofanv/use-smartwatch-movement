function [data_, data_rectify, mean_z] = ba_Arm_Wave_Dealing(data, Fs, i, j, cycle_time)
...
%ex = [31,42,56,106];
ex = 0;
if length(find(ex==(i*5+j))) > 0
    ang_z = ang_z;
else
    ang_z = baa_Angle_Pick(ang_z, mean_z);
end
...
%finddelay(s1,s2)
data_(:,1) = T(1000:length(Angz_filtered),:);
data_(:,2) = Angz_filtered(1000:length(Angz_filtered),:);
data_(:,3) = Angx_filtered(1000:length(Angz_filtered),:);
data_(:,4) = Angy_filtered(1000:length(Angz_filtered),:);
data_(:,5) = Accx_filtered(1000:length(Angz_filtered),:);
data_(:,6) = Accy_filtered(1000:length(Angz_filtered),:);
data_(:,7) = Accz_filtered(1000:length(Angz_filtered),:);
